...
# group by name location, eventcode
# how to sort by number of rows
# show list of urls with differing tones

apr14 %>% group_by(Actor1Name,Actor2Name, ActionGeo_Lat, ActionGeo_Long, EventCode) %>%
  summarise(n(),mean(AvgTone)) -> df1 %>% arrange(n()) -> new
  #mutate(domain = (domain(SOURCEURL))) 
df1
```
